...
`3.1.1` and `3.1.2` were both tagged and released with no `CHANGELOG.md`
entry. This locks the fix in place: the current package version must have a
matching heading so the next release cannot ship undocumented.
"""

import re
from pathlib import Path

from auth_sdk_m8 import __version__
...
def test_current_version_has_a_changelog_entry() -> None:
    """The version in `auth_sdk_m8.__version__` must head a CHANGELOG entry."""
    ...
    assert __version__ in headings, (
        ...
        f"version (auth_sdk_m8.__version__ = {__version__!r}); every "
